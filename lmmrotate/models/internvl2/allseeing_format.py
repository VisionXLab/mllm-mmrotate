# https://github.com/OpenGVLab/all-seeing/blob/main/all-seeing-v2/scripts_asmv2/visualize_rec.py
...
def loads_again(clean_item):
    pat = r"\[\d{1,4},\s?\d{1,4},\s?\d{1,4},\s?\d{1,4},\s?\d{1,4},\s?\d{1,4},\s?\d{1,4},\s?\d{1,4}\]"
    boxes = [json.loads(m) for m in re.findall(pat, clean_item)]
    return boxes
    ...
    for item in res:
        ...
        if item.startswith(BOX_START_TAG):
            ...
            try:
                ...
                clean_item = loads_again(clean_item)
